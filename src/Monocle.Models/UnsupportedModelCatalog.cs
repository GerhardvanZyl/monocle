...
/// <summary>
/// A model Monocle knows about but cannot run here. It occupies a row in the picker — greyed out,
/// with the reason spelled out — so the answer to "what else could score my photos?" lives in the
/// app rather than in a doc nobody opens (#9). Never becomes available and never scores; when one
/// of these gets a real path (an ONNX export, a GGUF for the Vulkan server), delete its entry here
/// and add a proper runner instead.
/// </summary>
public sealed class UnsupportedRunner : IModelRunner
{
    public UnsupportedRunner(ModelDescriptor descriptor) => Descriptor = descriptor;
    ...
    public Task<ModelScore> ScoreAsync(ScoringContext context, CancellationToken ct = default) =>
        throw new NotSupportedException($"{Descriptor.DisplayName} is not runnable here: {Descriptor.UnavailableReason}");
}
...
public static class UnsupportedModelCatalog
{
    ...
    // ROCm is Linux-only and the DirectML torch build is stale — which is why "CUDA/Linux" and
    // "no ONNX export" keep recurring below. Those are the honest blockers, not a verdict on the model.
    private const string PyTorchOnly =
        "PyTorch-only (pyiqa): no ONNX export, so no DirectML path on this AMD GPU. Runs on CPU at a few seconds a frame, or on an NVIDIA card.";

    public static readonly IReadOnlyList<ModelDescriptor> Models = new ModelDescriptor[]
    {
        // ---- No-reference technical quality (the pyiqa zoo) ----
        new()
        {
        ...
        new()
        {
            ...
            Description = "MLLM taught to rate images the way human annotators do, on the excellent/good/fair/"
                        + "poor/bad scale, with a written justification. Was wired up here and has been removed.",
            ...
            UnavailableReason = "Removed: its custom modelling code breaks on transformers 5.x (KeyError 'model'), "
                              + "and no GGUF exists, so the llama.cpp Vulkan route is closed too. Coming back "
                              + "means pinning an older transformers in the sidecar.",
        ...
        new()
        {
            ...
            Resource = ResourceKind.Gpu, OutputKind = ScoreKind.Aesthetic,
            UnavailableReason = "Wants CUDA + flash-attention; the reference stack is NVIDIA/Linux. llama.cpp has "
                              + "no GGUF support for the InternVL3 vision tower, so the Vulkan server can't host it.",
        ...
        new()
        {
            ...
            Resource = ResourceKind.Gpu, OutputKind = ScoreKind.Aesthetic,
            UnavailableReason = "No runner wired up yet. A GGUF + mmproj exists and llama.cpp supports it, so the "
                              + "existing Vulkan server could host it alongside Qwen2.5-VL.",
            InfoUrl = "https://huggingface.co/openbmb/MiniCPM-V-4_5-gguf",
        },

        // ---- Supporting detectors: they produce regions or maps, not a score ----
        new()
        {
            ...
            UnavailableReason = "No runner wired up yet. ONNX weights exist and would run on DirectML here; it "
                              + "needs a detector seam that returns regions, which Monocle doesn't have.",
        ...
        new()
        {
            ...
            UnavailableReason = "No runner wired up yet. ONNX weights exist (U2-Net especially) and would run on "
                              + "DirectML here; it needs a segmentation seam, which Monocle doesn't have.",
        ...
        new()
        {
            ...
            Resource = ResourceKind.Gpu, OutputKind = ScoreKind.Technical,
            UnavailableReason = "No runner wired up yet. ONNX weights exist and would run on DirectML here; like "
                              + "the detectors above it produces a map rather than a score, and the model seam "
                              + "has no shape for that.",
            InfoUrl = "https://huggingface.co/depth-anything/Depth-Anything-V2-Small",
        },
    };

    public static IEnumerable<UnsupportedRunner> BuildRunners() =>
        Models.Select(m => new UnsupportedRunner(m));
}
